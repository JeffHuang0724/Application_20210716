...
namespace Application_20210716
{
    public partial class VehicleLicenseTaxForm : Form
    {
        ...
        private decimal baseTax = new decimal();
        private DateTime userStartDate = new DateTime();
        private DateTime userEndDate = new DateTime();
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            ...
            // 取得控制項的值
            carType = this.comboBoxCarType.SelectedItem.ToString();
            displacement = this.comboBoxDisplacement.SelectedItem.ToString();
            // 如果勾選全期間，起訖日則以當年度來計算
            ...
            {
                userStartDate = this.dateTimePickerStart.Value;
                userEndDate = this.dateTimePickerEnd.Value;
            }

            // 取得稅額資訊
            TaxInfo taxInfo = new TaxInfo();
            baseTax = taxInfo.GetTaxInfo(carType, displacement);

            // 計算稅額，並將回傳結果印出
            Calculate calculate = new Calculate();
            this.txtResult.Text = calculate.GetCalResult(userStartDate, userEndDate, carType, displacement,baseTax);
        }
        ...
        private void Init()
        {
            ...
            baseTax = 0;
            userStartDate = new DateTime();
            userEndDate = new DateTime();
            // 介面初始化 - 使用期間初始化
